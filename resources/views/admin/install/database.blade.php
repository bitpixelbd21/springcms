...
<form method="POST" action="{{ route('install.saveDatabase') }}">
    <div class="container container-tight py-4" style="max-width: 30rem;">
        ...
        <h1>Database Configuration</h1>
            ...
            @csrf
            <label>DB Host</label>
            <input type="text" name="db_host" value="{{ old('db_host', session('db_host', '127.0.0.1')) }}" required>

            <label>DB Port</label>
            <input type="text" name="db_port" value="{{ old('db_port', session('db_port', '3306')) }}" required>

            <label>DB Name</label>
            <input type="text" name="db_database" value="{{ old('db_database', session('db_database')) }}" required>

            <label>DB Username</label>
            <input type="text" name="db_username" value="{{ old('db_username', session('db_username')) }}" required>

            <label>DB Password</label>
            <input type="password" name="db_password" value="{{ old('db_password', session('db_password')) }}">

            <input type="checkbox" name="create_database" id="create_database" {{ session('create_database') ? 'checked' : '' }}>
            <label for="create_database">Create Database if not exists</label>
